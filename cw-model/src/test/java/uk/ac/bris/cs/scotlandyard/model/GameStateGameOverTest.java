...
public class GameStateGameOverTest extends ParameterisedModelTestBase {
	...
	@Test public void testStartRoundShouldThrowIfGameAlreadyOver() {
	...
	@Test public void testGameNotOverWhenThereIsStillRoundsLeft() {
		var mrX = new Player(MRX, defaultMrXTickets(), 86);
		var blue = new Player(BLUE, defaultDetectiveTickets(), 85);
		GameState state = gameStateFactory.build(
				new GameSetup(standardGraph(), rounds(true, false, false)), mrX, blue);
	...
	@Test public void testGameOverAfterAllRoundsUsed() {
		var mrX = new Player(MRX, defaultMrXTickets(), 86);
		var blue = new Player(BLUE, defaultDetectiveTickets(), 85);
	...
	@Test public void testGameOverIfAllDetectivesStuck() {
		...
		GameState state = gameStateFactory.build(standard24RoundSetup(), mrX, blue, red);
	...
	@Test public void testDetectiveWinsIfMrXCornered() {
		...
		GameState state = gameStateFactory.build(standard24RoundSetup(), mrX, blue, red, green);
	...
	@Test public void testGameNotOverBeforeAnyRoundWithNonTerminatingConfiguration() {
		var mrX = new Player(MRX, defaultMrXTickets(), 86);
		var firstDetective = new Player(BLUE, defaultDetectiveTickets(), 108);
		GameState state = gameStateFactory.build(standard24RoundSetup(), mrX, firstDetective);
		assertGameIsNotOver(state);// game is not over with initial non-terminating setup
	}

	@Test public void testGameOverBeforeAnyRoundWithTerminatingConfiguration() {
